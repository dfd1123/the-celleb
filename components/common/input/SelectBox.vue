<template>
  <div ref="dropSelect" select-box :class="[`no-drag ${direction}`, { focus, error: error && !Boolean(dropListBox), readonly, disabled, open: Boolean(dropListBox), yet: !value }]">
  ...
  methods: {
    ...
    update () {
      ...
      if (list) {
        ...
        if (!val && !this.placeholder) { [val] = list } else if (!val) { [val] = list }
      }
...
[select-box] { .crop; .ib; .h(50); .-box; cursor: pointer; .rel; .bgc(white); .-a(white); .pointer;
  ...
  input[type="text"] { .rel; .z(2); .wh(100%); .pl(10); .pr(31); .tl;  .fs(14, 50px); .regular; .c(#787781); .ellipsis(1); .bgc(transparent); .pointer;
